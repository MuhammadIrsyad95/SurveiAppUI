...
interface Props {
  form: Form;
  onDeleted: (id: string) => void; // callback setelah delete sukses
  apiUrl: string;
}
...
export default function FormCard({ form, onDeleted, apiUrl }: Props) {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`${apiUrl}/Forms/${form.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed to delete form");

      MySwal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Form "${form.title}" has been deleted.`,
        timer: 1500,
  ...
  return (
    <div className="group bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-lg hover:border-blue-400 transition p-6 flex flex-col justify-between max-w-md w-full mx-auto">
      <Link href={`/Forms/${form.id}`} className="block cursor-pointer mb-4">
